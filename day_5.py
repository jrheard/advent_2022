...
def parse_instruction(instruction: str) -> Instruction:
    ...
    return Instruction(int(quantity), int(source) - 1, int(destination) - 1)


def parse_input() -> tuple[list[Stack], list[Instruction]]:
    ...
    for index in reversed(range(delimiter_index - 1)):
        line = lines[index]
        for stack_index in range(num_stacks):
            stack_value = line[stack_index * 4 + 1]
            if stack_value != " ":
                stacks[stack_index].append(stack_value)

    stack_list = [stacks[index] for index in range(num_stacks)]

    instructions = lines[delimiter_index + 1 :]
    return (
        stack_list,
...
def part_one() -> str:
    ...
    for instruction in instructions:
        for _ in range(instruction.quantity):
            value = stacks[instruction.source_index].pop()
            stacks[instruction.destination_index].append(value)
